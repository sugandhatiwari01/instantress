...
// Replace deprecated Mixtral with a supported Llama / Mistral model
const GROQ_MODELS = {
  DEFAULT: 'llama-3.1-8b-instant',      // Fast and current default model
  HIGH_CAP: 'llama-3.1-70b-versatile',  // Larger model for detailed outputs
  FALLBACK: 'llama2-70b-4096',          // Legacy fallback (still works)
};


function sanitizeAndTrimReadme(text, maxChars = 1000) {
  if (!text) return "";
  ...
  out = out.replace(/\[([^\]]+)\]\((http[s]?:\/\/[^\)]+)\)/g, '$1');

  // Remove long codeblocks (```...```)
...
async function safeGenerateContent(
  ...
  if (!groqClient) {
    console.warn("⚠️ Groq client not initialized - using local fallback");
  ...
  for (let i = 0; i < retries; i++) {
    try {
      ...
      // 🔍 Log the first part of the prompt text (so you know what’s being summarized)
      console.log("🧠 Prompt preview:", contents?.[0]?.parts?.[0]?.text?.slice(0, 200));
      ...
      // 🔍 Log the API response briefly
      const responseText = result?.data?.choices?.[0]?.message?.content?.trim();
      console.log("📝 Raw Groq response:", responseText?.slice(0, 300));

      if (!responseText) {
        console.warn("⚠️ No summary text returned — using fallback.");
        return await localGenerateContent({ contents, model });
      }
      ...
      console.error("Full error:", JSON.stringify(err, null, 2));
        ...
        // Try again with aggressively truncated prompts
        try {
          console.warn("Attempting a truncated retry (400).");
          const truncatedContents = contents.map(c => {
            const text = c.parts?.[0]?.text || '';
            return { role: c.role, parts: [{ text: text.slice(0, 600) + (text.length > 600 ? '...' : '') }] };
          });
          // call groq once more with smaller content
          const payload2 = {
            model: model || GROQ_MODELS.DEFAULT,
            messages: truncatedContents.map(c => ({ role: c.role, content: c.parts[0].text })),
            temperature: generationConfig.temperature || 0.5,
            max_tokens: generationConfig.max_tokens || 256,
            stream: false,
          };
          const result2 = await groqClient.post("/chat/completions", payload2);
          const responseText2 = result2?.data?.choices?.[0]?.message?.content?.trim();
          if (responseText2) return responseText2;
        } catch (retryErr) {
          console.error("Truncated retry failed:", retryErr?.message || retryErr);
        }
        return await localGenerateContent({ contents, model });
      }
...
async function summarizeReadme(repoName, readmeText) {
   const prompt = `
Summarize the GitHub project README in 2-3 concise technical bullet points:
- Focus only on features, technologies, and developer achievements.
- Do NOT include introductory lines like "Here are 3 bullet points".
- Return the summary in HTML format, using <ul> and <li> for bullets.
- Use <strong> tags to emphasize key features, technologies, or important phrases.
- Do NOT use Markdown syntax (**bold**), only HTML.
...
if (aiText) {
  try {
    // Try to parse JSON if valid JSON string
    if (aiText.trim().startsWith("{") || aiText.trim().startsWith("[")) {
      ...
      let cleanedText = aiText
        ...
        .replace(/function\s*\(.*\)\s*\{[\s\S]*?\}/g, "") // remove full JS function definitions
        .replace(/const\s+\w+\s*=\s*\{[\s\S]*?\};?/g, "") // remove object literals like const skills = {...}
      ...
      // If model added code-like keywords, strip them too
      cleanedText = cleanedText
        .replace(/\b(return|let|const|var|summary|skills|projects|education|work)\b.*$/i, "")
        .trim();

      // Use fallback summary if text still looks too short or like code
      if (!cleanedText || cleanedText.length < 40 || cleanedText.includes("(")) {
        cleanedText = `Passionate developer skilled in ${Object.values(categorizedSkills)
          .flat()
          .slice(0, 5)
          .join(", ")}. Experienced in building projects like ${bestProjects
          .map(p => p.name)
          .join(", ")}. Seeking to contribute to innovative software teams.`;
      }
    ...
    console.warn("⚠️ Groq returned invalid JSON, using fallback:", parseErr.message);
    aiOutput = {
      summary: `A dedicated developer skilled in ${Object.values(categorizedSkills)
        .flat()
        .slice(0, 5)
        .join(", ")}. Experienced in projects like ${bestProjects
        .map(p => p.name)
        .join(", ")}.`,
      enhancedExperience: workExperience,
    };
...
app.get("/api/test-summary", async (req, res) => {
  const sample = `This project is a front-end clone of the Amazon homepage built using HTML and CSS. It replicates the top navigation, a hero banner, product cards, and responsive layouts for mobile. Created as a practice project to improve layout and CSS skills.`;
  const safe = sanitizeAndTrimReadme(sample, 800);
  const summary = await safeGenerateContent({
    model: GROQ_MODELS.DEFAULT,
    contents: [{ role: "user", parts: [{ text: `Summarize this README in 2-3 concise technical bullet points:\n\n${safe}` }] }],
    generationConfig: { temperature: 0.2, max_tokens: 150 },
  });
...
const { githubUsername, name: providedName, categorizedSkills, bestProjects, summary, workExperience, education = {}, contactInfo, customSections } = data;

// Prefer an explicit name, then githubUsername, then a friendly default
const displayName = sanitizeInput(providedName || githubUsername || "Anonymous Developer");

// For GitHub display/link: prefer githubUsername; if missing, leave blank (no invalid link)
const displayGithub = githubUsername ? sanitizeInput(githubUsername) : "";
const name = sanitizeInput(data.name || githubUsername || "Anonymous Developer");

const sanitizedData = {
  // expose both displayName and github separately
  name: displayName,
  githubUsername: displayGithub || "YourName", // fallback display string (not used as link if empty)
      ...
      <body>
        <div class="resume">
          <div class="header">
            ...
            <div class="contact">
<div>
  ${displayGithub ? `<a href="https://github.com/${displayGithub}" aria-label="GitHub">@${displayGithub}</a>` : `<span class="small">GitHub: Not provided</span>`}
</div>              <div class="small">Email: ${sanitizedData.contactInfo.email}</div>
