...
:root{
  ...
  --brand:#2563eb;--brand-2:#60a5fa;--ring:#e5e7eb;--chip:#f3f4f6;
}
...
.nav{
  position:sticky;top:0;background:rgba(250,250,250,.9);
  backdrop-filter:saturate(1.2) blur(6px);
  border-bottom:1px solid var(--ring);z-index:50;
}
.nav .row{max-width:1100px;margin:0 auto;display:flex;gap:16px;
...
<body>
  ...
  <div class="wrap">
    ...
    <header class="header anchor" id="home">
      ...
      <div class="small">Software Developer • 
      ...
      <div class="grid projects">
        ${(bestProjects||[]).map(p => `
          <article class="project">
            <h3><a href="${esc(p.html_url || p.url || '#')}" target="_blank">${esc(p.name || 'Project')}</a></h3>
